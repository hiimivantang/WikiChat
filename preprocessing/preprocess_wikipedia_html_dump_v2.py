...
def save_checkpoint(checkpoint_file, processed_count, all_blocks, processed_articles=None):
    """Save the current processing state to a checkpoint file."""
    checkpoint_data = {
        "processed_count": processed_count,
        "block_count": len(all_blocks),
        "last_update": time.time(),
        "text_count": sum(1 for block in all_blocks if block.get("block_type") == "text"),
        "table_count": sum(1 for block in all_blocks if block.get("block_type") == "table"),
        "infobox_count": sum(1 for block in all_blocks if block.get("block_type") == "infobox"),
    }
    
    if processed_articles:
        checkpoint_data["processed_articles"] = processed_articles
    
    # Create a temporary file and then rename to avoid corruption if the process is killed during writing
    ...
    logger.info(f"Checkpoint saved: {processed_count} articles processed, {len(all_blocks)} blocks created")


def load_checkpoint(checkpoint_file):
    """Load the processing state from a checkpoint file."""
    ...
    try:
        ...
        logger.warning(f"Error loading checkpoint: {str(e)}. Starting from the beginning.")
        return 0, set()


def save_blocks_batch(output_path, blocks, batch_num):
    """Save a batch of blocks to a part file."""
    part_file = f"{output_path}.part{batch_num}"
    orjsonl.save(part_file, blocks)
    logger.info(f"Saved batch {batch_num} with {len(blocks)} blocks to {part_file}")
...
def process_articles(
    ...
    """Process articles from the input queue, with article_id for tracking processed articles."""
    ...
    while True:
        try:
            ...
            try:
                article_tuple = input_queue.get(timeout=QUEUE_TIMEOUT)
                if article_tuple is None:
                    logger.info(f"Worker {worker_id} received termination signal")
                    break
                    
                article, article_id = article_tuple
            except Exception as e:
                logger.warning(f"Worker {worker_id} timed out waiting for input: {str(e)}")
                continue
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Worker {worker_id} couldn't add to dead letter queue")
                continue
            ...
            for block in article_blocks:
                ...
                # Use a timeout for putting into the output queue, include article_id
                try:
                    output_queue.put((block, article_id), timeout=QUEUE_TIMEOUT)
                except Exception as e:
                    logger.warning(f"Worker {worker_id} timed out putting block in output queue: {str(e)}")
                    # If we can't put in the output queue, we'll add to dead letter queue
                    try:
                        dead_letter_queue.put(article, timeout=QUEUE_TIMEOUT)
                    except:
                        logger.error(f"Worker {worker_id} couldn't add to dead letter queue either")
            ...
            try:
                ...
                logger.error(f"Worker {worker_id} couldn't add to dead letter queue after KeyError")
        except Exception as e:
            logger.error(f"Worker {worker_id} encountered unexpected error: {str(e)}")
            try:
                dead_letter_queue.put(article, timeout=QUEUE_TIMEOUT)
            except:
                logger.error(f"Worker {worker_id} couldn't add to dead letter queue after error")
    
    # Signal completion
    try:
        output_queue.put((None, None), timeout=QUEUE_TIMEOUT)  # signal the end
        logger.info(f"Worker {worker_id} finished and sent termination signal")
    except:
        logger.error(f"Worker {worker_id} couldn't send termination signal")
...
def build_redirection_map(file_path: str) -> dict:
    """
    Build a map of article URLs to their redirects.
    
    Args:
        file_path: Path to the Wikipedia dump file.
        
    Returns:
        A dictionary mapping article URLs to sets of URLs that redirect to them.
    """
    ...
    logger.info(f"Built redirection map with {len(redirect_map)} entries")
    return redirect_map
...
def articles_without_disambiguation_or_redirections(
    ...
    """Feed articles to workers, skipping already processed ones if resuming from checkpoint."""
    processed_articles = processed_articles or set()
    ...
    try:
        for article in tarfile_loader(file_path):
            ...
            # Skip already processed articles if resuming from checkpoint
            ...
            try:
                ...
                queue.put((article, article_id), timeout=QUEUE_TIMEOUT)
                pbar.update(1)
                counter += 1
                if max_articles > 0 and counter >= max_articles:
                    break
            except Exception as e:
                logger.warning(f"Timeout putting article in queue: {str(e)}")
                ...
                try:
                    ...
                    logger.error(f"Failed to add article to queue even after retry")
    except Exception as e:
        ...
        for _ in range(num_workers):
            try:
                queue.put((None, None), timeout=QUEUE_TIMEOUT)  # signal end to all workers
            except:
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        ...
        help="Save checkpoint after processing this many blocks",
    )
    arg_parser.add_argument(
        "--batch_size",
        type=int,
        default=10000,
        help="Number of blocks to save in each batch file",
    )
    arg_parser.add_argument(
        "--main_loop_timeout",
        type=int,
        default=3600,  # 1 hour
        help="Maximum runtime for the main processing loop in seconds",
    )
    ...
    # Define checkpoint and redirection map file paths
    checkpoint_file = os.path.join(args.checkpoint_dir, f"{os.path.basename(args.output_path)}.checkpoint.json")
    redirection_map_file = os.path.join(args.checkpoint_dir, f"{args.language}_redirection_map.pkl")
    
    # Load checkpoint if resuming
    processed_count = 0
    processed_articles = set()
    if args.resume:
        processed_count, processed_articles = load_checkpoint(checkpoint_file)
    ...
    # Use Queue with timeout capability
    input_queue = Queue(maxsize=1000)
    output_queue = Queue(maxsize=1000)
    dead_letter_queue = Queue(maxsize=1000)
    ...
    reader_process = Process(
        ...
        args=(
            ...
            redirection_map,
            args.max_articles,
            processed_articles if args.resume else None,
        ),
    )
    ...
    counter = processed_count  # Start from the last processed count if resuming
    last_checkpoint_time = time.time()
    last_progress_time = time.time()

    # Set a timeout for the main loop
    main_loop_timeout = time.time() + args.main_loop_timeout
    
    try:
        while workers_finished < len(all_worker_processes):
            # Check if we've exceeded our timeout
            if time.time() > main_loop_timeout:
                logger.warning("Main processing loop timed out. Proceeding to save results.")
                break
                
            # Log progress periodically
            if time.time() - last_progress_time > 60:  # Log every minute
                logger.info(f"Progress: {counter} blocks processed, {workers_finished}/{len(all_worker_processes)} workers finished")
                last_progress_time = time.time()
                
            try:
                block_tuple = output_queue.get(timeout=QUEUE_TIMEOUT)
                if block_tuple is None:
                    workers_finished += 1
                    logger.info(f"Worker completed. {workers_finished}/{len(all_worker_processes)} workers finished")
                    continue
                    
                block, article_id = block_tuple
                ...
                all_blocks.append(json_block)
                current_batch.append(json_block)
                counter += 1
                
                # Save batch if it reaches the batch size
                if len(current_batch) >= args.batch_size:
                    save_blocks_batch(args.output_path, current_batch, batch_num)
                    current_batch = []
                    batch_num += 1
                
                # Save checkpoint periodically
                if counter % args.checkpoint_interval == 0:
                    save_checkpoint(
                        checkpoint_file, 
                        counter, 
                        all_blocks,
                ...
                logger.warning(f"Timeout or error getting from output queue: {str(e)}")
                ...
                # Save checkpoint if it's been a while
                if time.time() - last_checkpoint_time > 300:  # 5 minutes
                    save_checkpoint(
                        checkpoint_file, 
                        counter, 
                        all_blocks,
        ...
        logger.info("Processing interrupted by user. Saving current results.")
        ...
        # Terminate all processes if they're still running
        for p in all_worker_processes + [reader_process]:
            if p.is_alive():
    ...
    logger.info("Saving the final collection to %s", args.output_path)
    ...
    # Generate histogram if we have blocks
    if all_blocks:
        try:
            draw_and_save_histogram_log_bins(
            ...
            logger.error(f"Failed to generate histogram: {str(e)}")
    else:
        logger.warning("No blocks were processed, skipping histogram generation")
